    ...
    <div class="col-md-11" id="sidebar-right-section">
        ...
        <div class="container-fluid mt-1 d-flex" style="justify-content: center;">
            ...
            <form action="{{ route('Color.Store') }}" method="POST">
                @csrf
                @method('PUT')
                <x-form.inputbox type="hidden" name='id' value="{{  $color->id}}" placeholder='Color name'>
                </x-form.inputbox>
                <x-form.inputbox name='name' value="{{ old('name', $color->name) }}" placeholder='Color name'>
                </x-form.inputbox>
                <x-form.inputbox type="color" name='color' value="{{ old('color', $color->value) }}"
                    placeholder='Color name'>
                </x-form.inputbox>
                <select name="brand" class="my-2" style="width: 100%; height:38px;">
                    @foreach ($brandList as $brand)
                        <option value="{{ $brand->id }}" @if (old('brand',$brand->id) == $color->brand_id) {{ "selected"}} @endif>
                            {{ $brand->name}}</option>
                    @endforeach
                </select>
